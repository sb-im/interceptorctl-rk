...
- “低层电机”页可通过 RK `can0` 扫描唯一电机；自动配置会将非 1 的电机 ID 持久化改为 1 并复扫确认，再写入生产回零参数并严格回读验证；该流程不使能、不回零、不运动。
...
自动配置期间不要按实体开盖按钮。页面会显示初始/最终电机 ID、是否完成改址复扫、当前/目标参数、ACK 和回读验证结果；完整 JSON 仍保留在右侧命令日志中。改址前必须明确读到电机未使能，改址或复扫失败时不会继续写回零参数。
