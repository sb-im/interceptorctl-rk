...
- “系统与急停”页直接显示固件、整机、急停、UPS、温湿度、按键和统一开门角度状态。
- “舱门控制”页可选择并持久化 RK/API、实体按钮、急停释放共用的 90°/120°开盖档位，并可点击“从 MCU 回读当前角度”通过独立命令 ID 23 验证实际生效值。
...
自动配置期间不要按实体开盖按钮。每次扫描都会逐个探测 ID 1～32，不会发送广播指令。页面会显示初始/最终电机 ID、是否完成改址复扫、当前/目标参数、ACK 和回读验证结果；完整 JSON 仍保留在右侧命令日志中。电机上电默认使能，该状态只记录、不阻止配置；改址或复扫失败时不会继续写回零参数。
